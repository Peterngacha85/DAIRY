  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="section-container">
        <div className="section-header">
          <h2>Feed Stock</h2>
          <Link to="/feeds/new" className="primary-button">
            Add Feed Stock
          </Link>
        </div>
        <div className="feeds-grid">
          {dashboardData.recentFeeds.length === 0 ? (
            <p>No feed records yet. Add your first feed stock to get started!</p>
          ) : (
            dashboardData.recentFeeds.map((feed) => (
              <div key={feed._id} className={`feed-card ${feed.type}`}>
                <h3>
                  {feed.name}
